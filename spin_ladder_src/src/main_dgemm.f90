...
!************************************************************************
        ...
        real(kind=dp), dimension(1000)  :: jj_pair_r,CG_pair_r
        Integer :: size_pair_r
        INTEGER :: N_BS_state,N_center,N_engrad,N_pair,Hilbert_Dim,N_m
        ...
        allocate(M_cg(N_center,N_center,N_m,N_m,2*N_m)); M_cg=0.0_dp
        allocate(M_jj(N_center,N_center,N_m,N_m,2*N_m)); M_jj=0.0_dp
...
!!!!!!!!!!!!!!!! CALCULATE SPIN LADDER !!!!!!!!!!!!!!!!!
        ...
        allocate(Eigen_States_GS(1,Hilbert_Dim))
        allocate((Hilbert_Dim,1))
        allocate(Eigen_States_HS(1,Hilbert_Dim))
        allocate(Eigen_States_HS_Mul_H(Hilbert_Dim,1))
        allocate(Eigen_States_GS_Mul_H(Hilbert_Dim,1))
        ...
        GS_En= energies(1) - (Eigen_Values(Hilbert_Dim)-Eigen_Values(1))/fatt_au2cm1
...
!!!!!!!!!!!!!! CALCULATE FORCE of GROUND STATE !!!!!!!!!!!!!!!
        ...
        allocate(J_coupling_engrad(N_pair,N_engrad)); J_coupling_engrad=0.0_dp
        val=0.0_dp
        ...
        Eigen_States_GS(:,1)=Eigen_States_t(1,:)
        Eigen_States_HS(:,1)=Eigen_States_t(Hilbert_Dim,:)
        ...
        do nengrad=1,N_engrad
           ...
           do k=1,N_BS_state
              do p=1,N_pair
                 J_coupling_engrad(p,nengrad)=A_inverse_matrix(k,p)
              ...
              do i=1,Hilbert_Dim
                 do j=1,Hilbert_Dim
                    State_l(:)=Basis_Set(:,i)
                    State_r(:)=Basis_Set(:,j)
                    call engradDiffGroundHigh(State_l,State_r,Spin,J_coupling_engrad(:,nengrad),M_jj,M_cg,val)
              ...
              ! call ddot(Hilbert_Dim,H_diff_matrix,1,Eigen_States_GS,1)
              force_GS(k)=DOT_PRODUCT(Eigen_States_GS_Mul_H(:,1),Eigen_States_GS(1,:))

              ! do i = 1,Hilbert_Dim
                 ! Eigen_States_HS_Mul_H(i) = DOT_PRODUCT(Eigen_States_HS,H_diff_matrix(:,i))
              ! enddo
              call dgemm('N', 'N', Hilbert_Dim, 1, Hilbert_Dim, 1.0_dp, Eigen_States_HS, Hilbert_Dim+1, H_diff_matrix, Hilbert_Dim+1, 0.0_dp, Eigen_States_HS_Mul_H, Hilbert_Dim+1);
              force_HS(k)=DOT_PRODUCT(Eigen_States_HS_Mul_H(:,1),Eigen_States_HS(1,:))
              BS_contrib(k)=force_HS(k)-force_GS(k)
           enddo
           HS_engrad(nengrad) = engrad_matrix(1,nengrad)
           ! force = negative(engrad)
           do k = 1,N_BS_state
              force_diff(nengrad) = force_diff(nengrad) + BS_contrib(k)*engrad_matrix(k,nengrad)
           enddo
           GS_engrad(nengrad) = HS_engrad(nengrad) + force_diff(nengrad)
           write(unit_out,*) (k)
           write(unit_out,*) (force_diff(1))
        end do
