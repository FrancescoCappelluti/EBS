...
for i in Up-Up-Up-Up Down-Up-Up-Up Up-Down-Up-Up Up-Up-Down-Up Up-Up-Up-Down Down-Down-Up-Up Down-Up-Down-Up Up-Down-Down-Up
do
    # Extract from the output the MULLIKEN SPIN POPULATION for each Metal and the DFT Energy
#    echo $i >> M_tab.dat
    grep "    1     Mn       1" $DATAPATH/$i/out | awk '{printf("%10.6lf ",$7) }' >> M_tab.dat
    grep "    2     Mn       2" $DATAPATH/$i/out | awk '{printf("%10.6lf ",$7) }' >> M_tab.dat
    grep "    3     Mn       3" $DATAPATH/$i/out | awk '{printf("%10.6lf ",$7) }' >> M_tab.dat
    grep "    4     Mn       4" $DATAPATH/$i/out | awk '{printf("%10.6lf \n",$7) }' >> M_tab.dat

    grep "ENERGY| Total FORCE_EVAL " $DATAPATH/$i/out | awk '{printf("%20.12lf \n",$9) }' >> Energy.dat
